...
data(mtcars)
df_mtcars <- as.data.table(mtcars)

data(iris)
df_iris <- as.data.table(iris)
df_iris[, Species := NULL]

task_list <- list(
  mtcars = TaskRegr$new("mtcars", df_mtcars, target = "mpg"),
  iris = TaskRegr$new("iris", df_iris, target = "Sepal.Length")
)
...
results_mtcars <- validate_hyperparameters(task_list$mtcars, seed = 123)
print(results_mtcars)

results_iris <- validate_hyperparameters(task_list$iris, seed = 456)
print(results_iris)
...
fwrite(results_mtcars, "validation_mtcars.csv")
fwrite(results_iris, "validation_iris.csv")
